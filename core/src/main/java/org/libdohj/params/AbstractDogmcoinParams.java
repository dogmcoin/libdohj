...
/**
 * Common parameters for Dogmcoin networks.
 */
public abstract class AbstractDogmcoinParams extends NetworkParameters implements AuxPoWNetworkParameters {
    /** Standard format for the DOGM denomination. */
    public static final MonetaryFormat DOGM;
    /** Standard format for the mDOGM denomination. */
    public static final MonetaryFormat MDOGM;
    ...
    public static final int DOGM_TARGET_TIMESPAN = 4 * 60 * 60;  // 4 hours per difficulty cycle, on average.
    public static final int DOGM_TARGET_TIMESPAN_NEW = 60;  // 60s per difficulty cycle, on average. Kicks in after block 145k.
    public static final int DOGM_TARGET_SPACING = 1 * 60;  // 1 minute per block.
    public static final int DOGM_INTERVAL = DOGM_TARGET_TIMESPAN / DOGM_TARGET_SPACING;
    public static final int DOGM_INTERVAL_NEW = DOGM_TARGET_TIMESPAN_NEW / DOGM_TARGET_SPACING;

    /** Currency code for base 1 Dogmcoin. */
    public static final String CODE_DOGM = "DOGM";
    /** Currency code for base 1/1,000 Dogmcoin. */
    public static final String CODE_MDOGM = "mDOGM";
    /** Currency code for base 1/100,000,000 Dogmcoin. */
    ...
    static {
        DOGM = MonetaryFormat.BTC.noCode()
            .code(0, CODE_DOGM)
            .code(3, CODE_MDOGM)
            .code(7, CODE_KOINU);
        MDOGM = DOGM.shift(3).minDecimals(2).optionalDecimals(2);
        KOINU = DOGM.shift(7).minDecimals(0).optionalDecimals(2);
    }

    /** The string returned by getId() for the main, production network where people trade things. */
    public static final String ID_DOGM_MAINNET = "org.dogmcoin.production";
    /** The string returned by getId() for the testnet. */
    public static final String ID_DOGM_TESTNET = "org.dogmcoin.test";

    protected final int newInterval;
    protected final int newTargetTimespan;
    protected final int diffChangeTarget;

    protected Logger log = LoggerFactory.getLogger(AbstractDogmcoinParams.class);
    public static final int DOGMCOIN_PROTOCOL_VERSION_AUXPOW = 70003;
    public static final int DOGMCOIN_PROTOCOL_VERSION_CURRENT = 70004;

    private static final Coin BASE_SUBSIDY   = COIN.multiply(500000);
    private static final Coin STABLE_SUBSIDY = COIN.multiply(10000);

    public AbstractDogmcoinParams(final int setDiffChangeTarget) {
        super();
        genesisBlock = createGenesis(this);
        interval = DOGM_INTERVAL;
        newInterval = DOGM_INTERVAL_NEW;
        targetTimespan = DOGM_TARGET_TIMESPAN;
        newTargetTimespan = DOGM_TARGET_TIMESPAN_NEW;
     ...
     * Dogmcoin after block 145k uses 60 seconds.
     */
    ...
    public Coin getMaxMoney() {
        // TODO: Change to be Dogm compatible
        return MAX_MONEY;
    }
    ...
    /** Dogmcoin: Normally minimum difficulty blocks can only occur in between
    ...
    private boolean allowDigishieldMinDifficultyForBlock(final StoredBlock pindexLast, final Block pblock) {
        ...
        if ((!this.allowMinDifficultyBlocks())&&(pindexLast.getHeight()!=4253399))
            return false;
     ...
     * the weird cases for Dogmcoin such as testnet blocks which can be maximum
    ...
    public int getProtocolVersionNum(final ProtocolVersion version) {
        switch (version) {
            ...
            case CURRENT:
                return DOGMCOIN_PROTOCOL_VERSION_CURRENT;
            case MINIMUM:
            default:
                return DOGMCOIN_PROTOCOL_VERSION_AUXPOW;
        }
    }
    ...
    /**
     * Get the target time between individual blocks. Dogmcoin uses this in its
